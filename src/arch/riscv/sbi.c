    ...
    switch (event) {
        ...
        case HART_START: {
            ...
            if (cpu()->vcpu->arch.sbi_ctx.state == START_PENDING) {
                ...
                vcpu_writereg_raw(cpu()->vcpu, REG_A1, cpu()->vcpu->arch.sbi_ctx.priv);
...
static struct sbiret sbi_time_handler(unsigned long fid)
{
    ...
    uint64_t stime_value = vcpu_readreg_raw(cpu()->vcpu, REG_A0);
    if (RV32) {
        stime_value |= ((uint64_t)vcpu_readreg_raw(cpu()->vcpu, REG_A1)) << 32;
    }
...
static struct sbiret sbi_ipi_handler(unsigned long fid)
{
    ...
    unsigned long hart_mask = vcpu_readreg_raw(cpu()->vcpu, REG_A0);
    unsigned long hart_mask_base = vcpu_readreg_raw(cpu()->vcpu, REG_A1);
...
static struct sbiret sbi_base_handler(unsigned long fid)
{
    ...
    unsigned long extid = vcpu_readreg_raw(cpu()->vcpu, REG_A0);
...
static struct sbiret sbi_rfence_handler(unsigned long fid)
{
    ...
    unsigned long hart_mask = vcpu_readreg_raw(cpu()->vcpu, REG_A0);
    unsigned long hart_mask_base = vcpu_readreg_raw(cpu()->vcpu, REG_A1);
    unsigned long start_addr = vcpu_readreg_raw(cpu()->vcpu, REG_A2);
    unsigned long size = vcpu_readreg_raw(cpu()->vcpu, REG_A3);
    unsigned long asid = vcpu_readreg_raw(cpu()->vcpu, REG_A4);
...
{
    struct sbiret ret;
    vcpuid_t vhart_id = vcpu_readreg_raw(cpu()->vcpu, REG_A0);
    ...
    if (vhart_id == cpu()->vcpu->id) {
        ...
        if (vcpu == NULL) {
            ...
            if (vcpu->arch.sbi_ctx.state == STARTED) {
                ...
                vaddr_t start_addr = vcpu_readreg_raw(cpu()->vcpu, REG_A1);
                unsigned priv = (unsigned)vcpu_readreg_raw(cpu()->vcpu, REG_A2);
...
{
    struct sbiret ret;
    uint32_t suspend_type = (uint32_t)vcpu_readreg_raw(cpu()->vcpu, REG_A0);
...
{
    unsigned long extid = vcpu_readreg_raw(cpu()->vcpu, REG_A7);
    unsigned long fid = vcpu_readreg_raw(cpu()->vcpu, REG_A6);
    struct sbiret ret;
    ...
    vcpu_writereg_raw(cpu()->vcpu, REG_A0, (unsigned long)ret.error);
    vcpu_writereg_raw(cpu()->vcpu, REG_A1, (unsigned long)ret.value);
